...
export default function FormRegister() {
        ...
        const res = await toast.promise(
            emailjs.send(
                ...
                import.meta.env.VITE_EMAILJS_TEMPLATE_ID,
                {
                    ...
                    to_name: 'Palm Now',
                    message: 'Số điện thoại: ' + values.phone + ' \nLời nhắn: ' + values.message,
                },
                {
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className={cn('mt-9  w-full space-y-6 sm:space-y-8 ')}>
                <FormField
                        ...
                        <FormItem className="h-[50px]">
                            <FormControl>
                                <Input
                                    ...
                                    type="text"
                                    {...field}
                                    className="rounded-none border-l-0   border-r-0 border-t-0 border-black bg-transparent text-base tracking-wider text-secondary  duration-500 placeholder:text-muted-foreground focus-visible:border-b-secondary focus-visible:!ring-0 focus-visible:!ring-offset-0  focus-visible:placeholder:text-secondary"
                                />
                            </FormControl>
        ...
        /> */}
                ...
                <Button
                    ...
                    className="button-gradient !mt-20 flex w-full self-end font-bold uppercase tracking-widest  text-white hover:bg-secondary/80 sm:w-auto "
                >
                    {'Gửi yêu cầu'}
                    {form.formState.isSubmitting && <LoaderCircleIcon className="ml-3 animate-spin " />}
                </Button>
            </form>
        </Form>
    );
