...
function MobileSidebar() {
    ...
    return (
        <Sheet>
            <SheetTrigger className="justify-center  rounded-xl bg-background p-2 lg:hidden ">
                <Menu />
            </SheetTrigger>
            <SheetContent side={'right'} className="bg-glass w-56 p-4 pt-3">
                <a
                    href={'/'}
                    onClick={() => {
                        if (window.location.href == window.location.origin + '/') window.scrollTo({ top: 0 });
                    }}
                    className="cursor-pointer"
                >
                    <div className="flex items-center gap-x-2">
                        <svg
                            className="h-[55px] w-[35px] md:h-[70px] md:w-[50px] "
                            viewBox="0 0 676 863"
                            fill="none"
                        ...
                        <span className="text-gradient text-3xl font-bold">Palm</span>
                    </div>
                </a>
                        ...
                        return (
                            <SheetClose asChild key={index}>
                                <a
                                    ...
                                    href={navBar.href}
                                    className={cn(
                                        'group flex w-full cursor-pointer justify-start rounded-lg  p-3 text-sm font-medium text-white/50 transition duration-500 hover:text-white',
                                        pathName === navBar.href ? ' text-white' : '',
                                    )}
                                >
                    ...
                    <SheetClose className='w-full'>
                        <a href={'#contact'}>
                            <Button
                                variant={'default'}
                                className={cn(
                                    '  rounded-xl bg-background px-6  w-full  font-bold  capitalize  tracking-widest text-black duration-700 hover:text-white',
                                )}
                            >
                                Liên hệ
                            </Button>
